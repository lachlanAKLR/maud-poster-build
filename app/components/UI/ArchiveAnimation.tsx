    ...
    if (imagesLength > 0) {
      const intervalId = setInterval(() => {
        setVisibleIndex((prevIndex) => {
          ...
          if (nextIndex >= imagesLength) {
            clearInterval(intervalId);
            setTimeout(() => setAnimationDone(true), 300); // Wait for the last image to fade out
            return prevIndex; // Keep the last image visible until the timeout to hide it completes
          }
          return nextIndex;
        });
      }, 300);
